...
export default async function prod(chalk) {
    ...
    if (useWinTerminal) {
        ...
        const launched = tryLaunchInWindowsTerminal(
            "Electron-Flask Backend (prod)", backendDir, "python", ["production_run.py"]
        );
        if (launched) {
            ok(chalk, "Backend launched in a new tab");
        } else {
            info(chalk, "wt.exe unavailable — falling back to new CMD window...");
            launchInCmdWindow(backendDir, "python", ["production_run.py"]);
        ...
        const success = launchInUnixTerminal(
            "Electron-Flask Backend (prod)", backendDir, "python3", ["production_run.py"]
        );
        if (!success) {
    ...
    if (useWinTerminal) {
        ...
        const launched = tryLaunchInWindowsTerminal(
            "Electron-Flask Frontend (prod)", frontendDir, "npm", ["run", "dev"]
        );
        if (launched) {
