...
export default async function prod(chalk) {
    ...
    if (useWinTerminal) {
        ...
        if (launched) {
            ...
            launchInCmdWindow(backendDir, "python", ["production_run.py"], backendEnv);
        ...
        if (!success) {
            ...
            console.log(chalk.gray(`      cd "${backendDir}" && FLASK_ENV=production FLASK_PORT=${backendPort} python3 production_run.py`));
        } else {
    ...
    if (useWinTerminal) {
        ...
        if (!success) {
            ...
            console.log(chalk.gray(`      cd "${frontendDir}" && VITE_BACKEND_PORT=${backendPort} npm run dev -- --mode production --port ${frontendPort}`));
        } else {
